...
resource "aws_key_pair" "key_pair" {
  key_name   = "instance-test-key-Jenkins-mini-project"
  public_key = tls_private_key.example.public_key_openssh
}

# Create the security group with necessary ports
resource "aws_security_group" "sg" {
  name_prefix = "terraform-sg-Jenkins-mini-project"
...
resource "aws_instance" "Jenkins_instance-mini-project" {
  ...
  tags = {
    Name = "FlaskAppInstanceJenkinsMiniProject"
  }
}
